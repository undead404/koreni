import styles from './page.module.css';
export default function AboutPage() {
  return (
    <article className={styles.root}>
      <h1>Про проєкт "Корені"</h1>
      <section>
        <h2>Хто автор Коренів?</h2>
        <p>
          Привіт. Я&nbsp;– Віталій Перегончук, розробник ПЗ,&#20;
          <a
            href="https://www.linkedin.com/in/vitalii-perehonchuk-10570693/"
            target="_blank"
          >
            ось мій профіль на&nbsp;LinkedIn
          </a>
          , наприклад.
        </p>
        <p>
          Крім промислової розробки постійно займаюсь різноманітними
          хобі-проєктами. Серед актуальних:
        </p>
        <ul>
          <li>
            <a href="https://webdoky.org" target="_blank">
              WebDoky
            </a>
            &nbsp;– український переклад статей MDN
          </li>
          <li>
            <a href="https://you-must-hear.web.app" target="_blank">
              You Must Hear
            </a>
            &nbsp;– тут куди важче розповісти, що&nbsp;це таке 😂
          </li>
        </ul>
      </section>
      <section>
        <h2>Подяки</h2>
        <p>Уклінно дякую наступним особам і&nbsp;спільнотам:</p>
        <ul>
          <li>
            Володимиру Цибульському&nbsp;– за&nbsp;результати довгих років
            індексації
          </li>
          <li>
            <a href="https://t.me/+fF7z745gQBhhYmIy" target="_blank">
              Спільноті "Генеалогічне IT" в&nbsp;Telegram
            </a>
            &nbsp;– за&nbsp;концентрацію контексту та&nbsp;обговорення
          </li>
          <li>
            Проєкту "Індекси України"&nbsp;– за&nbsp;результати своєї праці
          </li>
          <li>
            Спільноті&nbsp;
            <a
              href="https://www.facebook.com/groups/425347154227812"
              target="_blank"
            >
              UAGenealogy на&nbsp;Facebook
            </a>
            &nbsp;і&#20;
            <a href="https://ukrgenealogy.com.ua/" target="_blank">
              УГФ&nbsp;– Українському Генеалогічному Форуму
            </a>
            &nbsp;– за&nbsp;формування українського генеалогічного дискурсу.
          </li>
          <li>
            Аліні Лістуновій&nbsp;– за&nbsp;PR з&nbsp;цим чудовим UX,
            за&nbsp;рефакторинг, коментарі та&nbsp;побажання.
          </li>
        </ul>
      </section>
      <section>
        <h2>Чому з'явилися Корені?</h2>
        <p>
          Вже деякий час я цікавлюся генеалогією; найдавніший відомий мій предок
          народився приблизно 1597 року. В процесі дослідження села Політанок,
          що&nbsp;в&nbsp;Жмеринському районі, взявся до&nbsp;суцільного
          дослідження генеалогії всього населеного пункту, а&nbsp;також&nbsp;–
          індексації всіх наявних генеалогічних матеріалів, яких, на&nbsp;щастя,
          збереглося чимало.
        </p>
        <p>
          Отже, я дослідив рідне село й&nbsp;маю купу таблиць Google
          Spreadsheets з індексаціями. У політанській Чудо-Михайлівській церкві,
          як&nbsp;це буває, хрестилися та&nbsp;вінчалися не&nbsp;тільки місцеві,
          а&nbsp;й:
        </p>
        <ul>
          <li>Захарія Іванів Стусь (у 1806 році), явно не&nbsp;місцевий</li>
          <li>
            Австрійські піддані Яків Йосипів Легуневич, Григорій Семенів
            Сенитович і&nbsp;Анна Іванова Дмитрюк
          </li>
          <li>Контролер винокурні Олександр Іванів Богданов</li>
          <li>Пруський підданий лютеранин Роберт Августів Врона</li>
          <li>Місцеве духовенство</li>
          <li>Шляхта</li>
          <li>а також купа селян навколишніх сіл</li>
        </ul>
        <p>
          В сленгу генеалогії таких осіб звуть "загуляками". Найбільше їх у
          великих містах і&nbsp;великих залізничних вузлах, проте траплялися
          і&nbsp;в глушині.
        </p>
        <p>
          Що&nbsp;робити з&nbsp;"загуляками"? Один варіант&nbsp;–
          викладати&nbsp;в&#20;
          <a
            href="https://www.facebook.com/groups/1828267990648440"
            target="_blank"
          >
            Загуляк на&nbsp;Facebook
          </a>
          . Іще можна викладати таблиці на&nbsp;форуми й&nbsp;у&nbsp;соцмережах,
          сподіваючись, що&nbsp;на&nbsp;таблицю натрапить саме той, хто треба.
        </p>
        <p>
          Я вирішив піти далі й&nbsp;розробити вебзастосунок, який агрегував би
          таблиці від&nbsp;різних дослідників. Додатковою мотивацією є
          аби&nbsp;таблиці не лежали без&nbsp;діла&nbsp;– раптом ще комусь
          допоможуть?
        </p>
      </section>
      <section>
        <h2>Як можна допомогти?</h2>
        <p>
          Якщо маєте таблиці-індексації&nbsp;– присилайте мені&nbsp;на&#20;
          <a href="mailto:brute18@gmail.com">brute18@gmail.com</a>. Корені
          спроєктовані таким чином, щоб додавання нової таблиці займало не
          більше 5 хвилин, тому додавання нових даних відбудеться швидко.
        </p>
        <p>
          Якщо ж бажаєте допомогти з&nbsp;кодом&nbsp;– приходьте&nbsp;на&#20;
          <a href="https://github.com/undead404/koreni" target="_blank">
            Корені в&nbsp;Github
          </a>
          , реєструйте проблеми, присилайте PR!
        </p>
        <p>
          І&nbsp;– якщо Корені допомогли в&nbsp;якихось пошуках&nbsp;– напишіть
          мені&nbsp;на&#20;
          <a href="mailto:brute18@gmail.com">brute18@gmail.com</a> обов'язково:
          я б дуже хотів знати, що&nbsp;моя праця допомогла комусь.
        </p>
      </section>
    </article>
  );
}
