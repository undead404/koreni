...
export default function ContributeForm({
  ...
  const locationInputReference = useRef<HTMLInputElement>(null);
  ...
  const handleLocationSelect = useCallback((coordinates: [number, number]) => {
    if (locationInputReference.current) {
      const value = coordinates.join(', ');
      locationInputReference.current.value = value;
      locationInputReference.current.setCustomValidity(''); // Reset any previous error
      // Trigger blur logic to save state
      const data = restoreContributePageState() || {};
      data['location'] = value;
      saveContributePageState(data);
    }
    ...
    (event: FocusEvent<HTMLInputElement>) => {
      ...
      if (
        ...
        input.setCustomValidity(
          'Будь ласка, введіть коректні координати (широта: від -90 до 90, довгота: від -180 до 180)',
        );
      } else {
        input.setCustomValidity('');
        handleInputBlur(event);
  ...
  const handleLocationChange = useMemo(
    () =>
      _.throttle((event: ChangeEvent<HTMLInputElement>) => {
                ...
                {knownLocations
                  ...
                  .map((loc) => (
                    <button
                      ...
                      onClick={() => handleLocationSelect(loc.coordinates)}
                    >
                      {loc.title}
                    </button>
                  ))}
